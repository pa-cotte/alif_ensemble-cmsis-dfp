...
static int32_t ILI9806E_BL_LED_Init(void)
{
	...
	ret = GPIO_Driver_BLED->SetDirection(RTE_ILI9806E_PANEL_BL_LED_PIN_NO, GPIO_PIN_DIRECTION_OUTPUT);
	if(ret != ARM_DRIVER_OK)
		return ARM_DRIVER_ERROR;
...
static int32_t ILI9806E_BL_LED_Control(uint8_t state)
{
	...
	if(state == ENABLE)
	{
		ret = GPIO_Driver_BLED->SetValue(RTE_ILI9806E_PANEL_BL_LED_PIN_NO, GPIO_PIN_OUTPUT_STATE_HIGH);
		if(ret != ARM_DRIVER_OK)
			return ARM_DRIVER_ERROR;
	}
	else
	{
		ret = GPIO_Driver_BLED->SetValue(RTE_ILI9806E_PANEL_BL_LED_PIN_NO, GPIO_PIN_OUTPUT_STATE_LOW);
		if(ret != ARM_DRIVER_OK)
			return ARM_DRIVER_ERROR;
	}
...
static DISPLAY_PANEL_FRAME_INFO ILI9806E_display_frame_info =
{
...
static DISPLAY_PANEL_INFO ILI9806E_panel_info =
{
	.max_bitrate = RTE_ILI9806E_PANEL_MAX_BITRATE_MBPS
};

extern DISPLAY_PANEL_DEVICE ILI9806E_display_panel;
DISPLAY_PANEL_DEVICE ILI9806E_display_panel =
{
	.Ops = &ILI9806E_display_ops,
	.Info = &ILI9806E_panel_info,
	.Frame_info = &ILI9806E_display_frame_info
};
