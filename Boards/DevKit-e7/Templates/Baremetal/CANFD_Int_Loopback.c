...
volatile bool canfd_error                = false;
...
void cb_object_event(uint32_t obj_idx, uint32_t event)
{
    if((event & ARM_CAN_EVENT_RECEIVE) || (event & ARM_CAN_EVENT_RECEIVE_OVERRUN))
    {
        ...
        if(obj_idx == rx_obj_id)
        {
            ...
            if(CANFD_instance->MessageRead(obj_idx, &rx_msg_header, rx_data, rx_msg_size) != ARM_DRIVER_OK)
            {
                rx_msg_error = true;
            }
            else
...
/**
 * @fn      void canfd_lbi_demo_thread_entry(void)
 * @brief   CANFD Internal Loopback Demo
 * @note    none
 * @param   none
 * @retval  none
 */
void canfd_lbi_demo_thread_entry(void)
{
